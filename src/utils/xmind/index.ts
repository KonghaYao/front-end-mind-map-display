...
export class XMindEmbedViewer {
    ...
    constructor(args: {
        ...
        const iframeEventChannelController = new IframeEventChannelController(
            iframeController,
            new URL(url).origin
        );
        ...
        iframeEventChannelController.addEventListener<string>(
            "sheet-switch",
            (payload) => (this.internalState.currentSheetId = payload)
        );
        iframeEventChannelController.addEventListener<number>(
            "zoom-change",
            (payload) => (this.internalState.zoomScale = payload)
        );
        iframeEventChannelController.addEventListener<Sheet[]>(
            "sheets-load",
            (payload) => (this.internalState.sheets = payload)
        );
